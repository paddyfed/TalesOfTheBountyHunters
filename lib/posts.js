...
export async function getFilteredPostsData(year) {
  ...
  if(yearFilter && monthFilter) {
    return allPostsData.filter(val => val.date.startsWith(`${yearFilter}-${monthFilter}`))
  }
...
function getAllYearsAndMonths() {
  const fileNames = fs.readdirSync(postsDirectory);
  const allYears = fileNames.map((fileName) => {
    return parseInt(fileName.slice(0, 7));
  });
...
export function getAllYearsForStaticProps() {
  ...
  const returnSortable = Object.entries(yearCount)
    .sort((a,b) => b[0] - a[0]);
...
export function getAllTags() {
  ...
  const sortable = Object.entries(tagCount)
